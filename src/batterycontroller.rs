...
pub struct BatteryController {
    pub adc: ads1x1x::Ads1x1x<ads1x1x::interface::I2cInterface<linux_embedded_hal::I2cdev>, 
    ads1x1x::ic::Ads1115, ads1x1x::ic::Resolution16Bit, ads1x1x::mode::OneShot>,
    _max_vol_ch0: f32,
    _max_vol_ch1: f32,
    _max_vol_ch2: f32,
}
impl BatteryController {
    ...
    pub fn read_adc(&mut self, channel: ADCCHANNEL) -> i16
    {
        match channel
        {
            ADCCHANNEL::Channel0 => return nb::block!(self.adc.read(&mut channel::SingleA0)).unwrap(),
            ADCCHANNEL::Channel1 => return nb::block!(self.adc.read(&mut channel::SingleA1)).unwrap(),
            ADCCHANNEL::Channel2 => return nb::block!(self.adc.read(&mut channel::SingleA2)).unwrap(),
            ADCCHANNEL::Channel3 => return nb::block!(self.adc.read(&mut channel::SingleA3)).unwrap(),
        }
    }
    ...
    {
        match cell {
            BATTERYCELL::Firstcell => return self.read_adc(ADCCHANNEL::Channel2) as f32 * (self._max_vol_ch2 / (i16::MAX) as f32),

            BATTERYCELL::Secondcell => return self.read_adc(ADCCHANNEL::Channel1) as f32 * (self._max_vol_ch1 / (i16::MAX) as f32) - 
            self.read_adc(ADCCHANNEL::Channel2) as f32 * (self._max_vol_ch2 / (i16::MAX) as f32),

            BATTERYCELL::Trindcell => return self.read_adc(ADCCHANNEL::Channel0) as f32 * (self._max_vol_ch0 / (i16::MAX) as f32) - 
            self.read_adc(ADCCHANNEL::Channel1) as f32 * (self._max_vol_ch1 / (i16::MAX) as f32),
        }
    }
    ...
    {
        return self.read_adc(ADCCHANNEL::Channel0) as f32 * (self._max_vol_ch0 / (i16::MAX) as f32);
    }
